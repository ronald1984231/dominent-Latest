        ...
        `,
          [
            ...
            updateData.lastWhoisCheck
              ? new Date(updateData.lastWhoisCheck)
              : null,
  ...
  try {
    ...
    `,
      [
        ...
        "Just now",
        monitoringUpdate.lastWhoisCheck
          ? new Date(monitoringUpdate.lastWhoisCheck)
          : null,
        monitoringUpdate.lastSslCheck
          ? new Date(monitoringUpdate.lastSslCheck)
          : null,
        monitoringUpdate.expiry_date
          ? new Date(monitoringUpdate.expiry_date)
          : null,
        monitoringUpdate.ssl_expiry
          ? new Date(monitoringUpdate.ssl_expiry)
          : null,
...
export const updateDomainFromCron = async (
  domainId: string,
...
export const getAllDomainsForCron = async (): Promise<Domain[]> => {
  try {
    const result = await db.query(
      "SELECT * FROM domains WHERE is_active = true ORDER BY created_at DESC",
    );
