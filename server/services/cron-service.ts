...
export class CronService {
  ...
  private startDomainMonitoring() {
    ...
    const schedule =
      process.env.NODE_ENV === "development" ? "*/10 * * * *" : "0 2 * * *";
    ...
    this.monitoringJob = cron.schedule(
      ...
      async () => {
        if (this.isMonitoringRunning) {
          console.log(
            "⏭️ Skipping monitoring run - previous run still in progress",
          );
          return;
        }
  ...
  private async runDomainMonitoring() {
    ...
    for (let i = 0; i < domains.length; i += batchSize) {
      ...
      console.log(
        `Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(domains.length / batchSize)} (${batch.length} domains)`,
      );

      // Process batch in parallel with delay between domains
      const batchPromises = batch.map(
        (domain, index) => this.monitorDomainWithDelay(domain, index * 2000), // 2 second delay between each domain
      );
      ...
      batchResults.forEach((result, index) => {
        if (result.status === "fulfilled") {
          ...
          results.failed++;
          console.error(
            `❌ ${batch[index].domain} - monitoring failed:`,
            result.reason,
          );
        }
      });
    ...
    console.log(
      `📈 Monitoring completed: ${results.success} successful, ${results.failed} failed out of ${results.total} domains`,
    );
   ...
   */
  private async monitorDomainWithDelay(
    domain: Domain,
    delay: number,
  ): Promise<void> {
    if (delay > 0) {
   ...
   */
  private async updateDomainData(
    domainId: string,
    updateData: any,
  ): Promise<void> {
    try {
      ...
      const domainUpdates = {
        ...
        ...(updateData.lastWhoisCheck && {
          lastWhoisCheck: updateData.lastWhoisCheck,
        }),
        ...(updateData.lastSslCheck && {
