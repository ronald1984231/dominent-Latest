  ...
  useEffect(() => {
    ...
    console.warn = (...args) => {
      ...
      if (message.includes('createRoot')) {
        setCreateRootWarnings(prev => [...prev, `${new Date().toLocaleTimeString()}: ${message}`]);
      }
    ...
    console.error = (...args) => {
      const message = args.join(' ');
      if (message.includes('Failed to load dashboard data') || message.includes('Failed to fetch')) {
  ...
  const testMultipleRequests = async () => {
    ...
    try {
      ...
      const promises = Array(5).fill(0).map(() => 
        safeFetchJson("/api/internal/dashboard")
      );
              ...
              {dashboardData ? (
                <div className="space-y-2">
                  <div className="text-sm">
                    <strong>Total Domains:</strong> {dashboardData.stats?.totalDomains || 0}
                  </div>
                  <div className="text-sm">
                    <strong>Expiring Domains:</strong> {dashboardData.stats?.expiringDomains || 0}
                  </div>
                  <div className="text-sm">
                    <strong>Expiring Certificates:</strong> {dashboardData.stats?.expiringCertificates || 0}
                  </div>
                  <div className="text-sm">
                    <strong>Online Domains:</strong> {dashboardData.stats?.onlineDomains || 0}
                  </div>
                ...
                <p className="text-gray-500">No data loaded yet. Click "Test Dashboard API" to fetch data.</p>
              )}
            </CardContent>
          </Card>
        </div>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div className="text-center">
                <div className={`w-4 h-4 rounded-full mx-auto mb-2 ${
                  createRootWarnings.length === 0 ? 'bg-green-500' : 'bg-red-500'
                }`}></div>
                <div className="text-sm">
                  <div className="font-medium">CreateRoot Fix</div>
                  <div className="text-gray-600">
                    {createRootWarnings.length === 0 ? 'Working' : `${createRootWarnings.length} warnings`}
                  </div>
                </div>
              </div>
              ...
              <div className="text-center">
                ...
                <div className="text-sm">
                  ...
                  <div className="text-gray-600">
                    {fetchErrors.length === 0 ? 'Working' : `${fetchErrors.length} errors`}
                  </div>
                </div>
              </div>
